...
from models.fuse import ChannelGate, SA_Enhance
...
class Enhance(nn.Module):
    def __init__(self, in_channels, out_channels):
        ...
        self.conv1 = nn.Conv2d(out_channels * 2, out_channels, kernel_size=3, stride=1, padding=1)
        self.conv2 = nn.Conv2d(out_channels * 2, out_channels, kernel_size=3, stride=1, padding=1)
        self.conv3 = nn.Conv2d(out_channels * 2, out_channels, kernel_size=3, stride=1, padding=1)
        self.sa_enhance = SA_Enhance(dim=out_channels, reduction=4, num_heads=4)

        self.conv_tail = Depth_conv(out_channels, in_channels)
        self.ChannelGate = ChannelGate(out_channels*2, pool_types=['avg'], reduction_ratio=4)
    # MDT_nomasked_patch4_img256_8V100_StepLR_SNRTransformerBlock_EntropyEnhance_两两交叉
    def forward(self, x, condition):

        b, c, h, w = x.shape # 24, 3, 128, 128
        residual = x

        x = self.conv_head(x) # torch.Size([24, 64, 128, 128])
        x_HL, x_LH, x_HH = x[:b//3, ...], x[b//3:2*b//3, ...], x[2*b//3:, ...]
        # x_HL: torch.Size([8, 64, 128, 128])  x_LH: 8, 64, 128, 128 x_HH: torch.Size([8, 64, 128, 128])

        if condition is not None:
            condition = F.interpolate(condition, size=(h,w), mode='bilinear', align_corners=True)
            condition = self.condtion_conv(condition)

        cond_LH = self.sa_enhance(condition, x_LH) # 8, 64, 128, 128
        cond_HL = self.sa_enhance(condition, x_HL) # 8, 64, 128, 128
        cond_HH = self.sa_enhance(condition, x_HH) # 8, 64, 128, 128

        x_HL = self.dilated_block_HL(cond_HL)
        x_LH = self.dilated_block_LH(cond_LH)
        x_HH = self.dilated_block_HH(cond_HH)
        # print(self.ChannelGate( torch.cat((x_HL, x_LH), dim=1) ).shape)
        x_LH = x_LH + self.conv1(self.ChannelGate( torch.cat((x_HL, x_HH), dim=1) )) 
        x_HL = x_HL + self.conv2(self.ChannelGate( torch.cat((x_LH, x_HH), dim=1) ))
        x_HH = x_HH + self.conv3(self.ChannelGate( torch.cat((x_HL, x_LH), dim=1) ))
